...
#   x:      $t3    (helper varibale which stores sq*sq)
#   rem:    $t4    (helper variable which stores n%i) 
#   max_sq: $t5    (helps to avoid overflow in x incase of big input for n) 
#
main:
...
algo:
    ...
    li $t1, 1       # variable sq
    li $t5, 46340   # floor of square root of max 32 bit signed integer 

while:
    mul $t3, $t1, $t1   # x = sq*sq
    bgt $t3, $s1, endWhile  # if x>n goto endWhile
    addi $t1, $t1, 1        # sq = sq + 1
    bgt $t1, $t5, endWhile  # if sq > max_sq goto endwhile
